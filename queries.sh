...
echo   "$($PSQL "SELECT DISTINCT(name) FROM teams t FULL JOIN games g ON t.team_id = g.winner_id FULL JOIN games g2 ON t.team_id = g2.opponent_id WHERE (g.round = 'Eighth-Final' AND g.year = 2014)  OR (g2.round = 'Eighth-Final' AND g2.year = 2014) ORDER BY name")"
